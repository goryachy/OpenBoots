...
CREATE TABLE IF NOT EXISTS receiving_events (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id INTEGER NOT NULL REFERENCES receiving_sessions(id) ON DELETE CASCADE, product_id INTEGER NOT NULL REFERENCES products(id), quantity INTEGER NOT NULL CHECK(quantity > 0), barcode TEXT NOT NULL, created_at TEXT NOT NULL);
...
const hash = (value) => crypto.scryptSync(value, 'openboots-demo-salt', 32).toString('hex');
const seed = db.transaction(() => {
  if (!db.prepare('SELECT 1 FROM users LIMIT 1').get()) db.prepare('INSERT INTO users(username,password_hash,role) VALUES (?,?,?)').run('admin', hash('admin'), 'operator');
  if (!db.prepare('SELECT 1 FROM locations LIMIT 1').get()) {
    const add = db.prepare('INSERT INTO locations(name) VALUES (?)');
    ['Main Warehouse', 'Showroom', 'Warehouse 2'].forEach((name) => add.run(name));
  }
  if (!db.prepare('SELECT 1 FROM products LIMIT 1').get()) {
    const add = db.prepare('INSERT INTO products(name,brand,model,article,color,sizes_json,pairs_per_box,sale_price_cents,barcode,internal_barcode,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)');
    const product = add.run('9060', 'New Balance', '9060', 'NB9060-BLK', 'Black', JSON.stringify([{ size: '36', quantity: 1 }, { size: '37', quantity: 1 }, { size: '38', quantity: 2 }, { size: '39', quantity: 2 }, { size: '40', quantity: 1 }, { size: '41', quantity: 1 }]), 8, 125000, '1234567890123', 'INV-000001824', now());
    ...
    addStock.run(product.lastInsertRowid, loc[0].id, 18); addStock.run(product.lastInsertRowid, loc[1].id, 2); addStock.run(product.lastInsertRowid, loc[2].id, 7);
    db.prepare('INSERT INTO product_aliases(barcode,product_id,source) VALUES (?,?,?)').run('1234567890123', product.lastInsertRowid, 'manufacturer');
  }
});
seed();
